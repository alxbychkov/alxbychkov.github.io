...
var number = document.querySelector('.info__numbers');
var arrow = document.querySelector('.chart__arrow');
var boxes = document.querySelectorAll('.item__cb');
var checked = document.querySelectorAll('.item__cb[checked]');
var length = boxes.length;
var data = checked.length;

var printNumber = function printNumber(num) {
  var percent = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 100;
  return Math.round(num / length * percent);
};

function printArrow(num, percent) {
  var deg = printNumber(num, percent) * 2 - percent;
  return "rotate(".concat(deg, "deg)");
}

function count(field, num) {
  var time = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 40;
  var quantity = +field.innerText;
  var timer = setInterval(function () {
    // eslint-disable-next-line no-unused-expressions
    quantity < printNumber(num) ? quantity++ : quantity--;
    field.innerHTML = quantity;

    if (quantity === printNumber(num)) {
      clearInterval(timer);
    }
  }, time);
}
...
count(number, data, 5);
arrow.style.transform = printArrow(data, 90);
boxes.forEach(function (box) {
  box.addEventListener('change', function (e) {
    // eslint-disable-next-line no-unused-expressions
    e.target.checked ? data++ : data--;
    arrow.style.transform = printArrow(data, 90);
    count(number, data);
  });
});
